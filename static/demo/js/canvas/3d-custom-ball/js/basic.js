...
PreLoading.prototype.preLoadImg = function(imgArr){
	var c = this;
	var imgArr = (typeof imgArr != "object") ? [imgArr] : imgArr;  
	for(var a = imgArr, b = 0, d = a.length; b < d; b++){
      var e = new Image();
      e.src = a[b];
      e.onload = c.countLoadImgNum(a);
	}
};
PreLoading.prototype.countLoadImgNum = function(imgArr){
	var c = this;
	c.loadImgNum++;
	if(c.loadImgNum == imgArr.length){
		c.loadImgCom = true;
		console.log("loadImgResult-" + c.loadImgNum);
	}
};
PreLoading.prototype.preLoadAudio=function(audioArr){
	var c = this;
	var audioArr = (typeof audioArr != "object") ? [audioArr] : audioArr;  
	for(var a = audioArr, b = 0, d = a.length; b < d; b++){
		var e = new Audio();
		e.src = a[b];
		e.onloadedmetadata = c.countLoadAudioNum(a);
	}
};
PreLoading.prototype.countLoadAudioNum=function(audioArr){
	var c = this;
	c.loadAudioNum++;
	if(c.loadAudioNum == audioArr.length){
		c.loadAudioCom = true;
